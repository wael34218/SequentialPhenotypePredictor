...
class Ensemble(BinaryPredictor):
    def __init__(self, filename, window=10, size=600, decay=5, stopwords=0, threshold=0.5,
        ...
        self._models = ["cfilter", "cbowsim", "skipgram"]

        self._model_predictions = {d: {m: [] for m in self._models} for d in self._diags}
        self._model_actual = {d: [] for d in self._diags}
        ...
        with open(filename) as f:
            for line in f:
                ...
                for diag in self._diags:
                    self._model_predictions[diag]["cfilter"].append(cf_preds[diag])
                    self._model_predictions[diag]["cbowsim"].append(cbow_preds[diag])
                    self._model_predictions[diag]["skipgram"].append(skip_preds[diag])
                    self._model_actual[diag] += [int(diag in actual)]

        for diag in self._diags:
            import pdb; pdb.set_trace()
            fpr, tpr, thresholds = metrics.roc_curve(
                self._model_actual[diag], [self._model_predictions[diag]["cbowsim"],
                                           self._model_predictions[diag]["cfilter"],
                                           self._model_predictions[diag]["skipgram"]])
